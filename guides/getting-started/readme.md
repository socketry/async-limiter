...
1. **Convoy Effect Prevention**: Quick `timeout: 0` operations are not blocked by slow operations.
2. **Deadline Tracking**: Total operation time never exceeds specified timeout.
3. **Fast Path Optimization**: Expired deadlines are detected immediately.
4. **Minimal Overhead**: Unlimited operations have near-zero overhead.

## Best Practices

1. **Start simple**: Use `Generic` or `Limited` without timing strategies initially.
2. **Add timing gradually**: Introduce timing strategies as scaling needs emerge.
3. **Validate costs**: Use appropriate cost values that don't exceed timing strategy capacity.
4. **Handle timeouts**: Always check return values from `acquire(timeout:)` calls.
5. **Use blocks**: Prefer block-form acquisition for automatic cleanup.
6. **Monitor performance**: Use timing strategies' monitoring methods (`level`, `can_acquire?`).

This guide provides the foundation for using the modern `async-limiter` architecture effectively in your Ruby applications.